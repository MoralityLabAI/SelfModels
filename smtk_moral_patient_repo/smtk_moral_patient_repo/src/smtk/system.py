...
from .moral_patient import WelfareState
...
class SMTKSystem(nn.Module):
    def __init__(
        ...
        # MV dims
        self.d_mv_pre = d_I + d_S + d_C + d_P + d_eta + 4  # + welfare state
    ...
    def step(
        ...
        # update welfare from governance / environment events
        ...
        mv_pre = torch.cat([I_vec, S_vec, C_vec, P_vec, eta, W_vec], dim=0)

        # action
        # right-of-refusal proxy: if welfare/autonomy fall below threshold, emit REFUSE_TOKEN
        if self.welfare.should_refuse(t):
        ...
        rec = LedgerRecord(
            ...
            meta={"temperature": temperature, "debt": debt, "violations": violations, "entropy": float(entropy_from_logits(logits).item()), "welfare": self.welfare.welfare, "autonomy": self.welfare.autonomy, "integrity": self.welfare.integrity, "pain": self.welfare.pain}
        )
